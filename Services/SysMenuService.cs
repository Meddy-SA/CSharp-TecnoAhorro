...
public class SysMenuService(Context context) : ISysMenu
{
  ...
  {
    var response = new ResponseDTO<string> { Success = false };
    try
    {
      var categories = await context.SysMenuCategories
          .Include(c => c.Items)
      ...
      foreach (var category in categories)
      {
        category.Items = [.. category.Items
              .Where(i => i.Roles == null || i.Roles.Intersect(rolesToFilter).Any())
  ...
  public async Task<bool> AddCategoryAsync(string name, List<RolesEnum> roles)
  {
    try
    {
      SysMenuCategory menuCategory = new()
      {
        Name = name,
        Roles = roles
      };
  ...
  private async Task AddChildrenAsync(ICollection<SysMenuItem> items, List<RolesEnum> rolesFilter)
  {
    ...
    var children = await context.SysMenuItems
        .Where(i => childIds.Contains(i.SysMenuItemId ?? 0) &&
