...
public class CategoryService(Context context, IMapper mapper) : ICategoryHandle
{
  ...
  {
    var response = new ResponseDTO<List<CategoryDTO>> { Success = false };
    try
    {
  ...
  {
    var response = new ResponseDTO<CategoryDTO> { Success = false };
    try
    {
  ...
  public async Task<ResponseDTO<CategoryDTO>> UpdateAsync(int id, CategoryDTO categoryUpdate)
  {
    ...
    try
    {
      ...
      {
        response.Success = false;
        response.Error = "El ID en la ruta no coincide con el ID en el cuerpo de la solicitud";
        return response;
      }
